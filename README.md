...
Each sub-workspace is built independently with `colcon`.
...
Source all install overlays before running:
...
The Livox driver requires **Livox-SDK2** to be installed on the host.  
Follow the instructions at: <https://github.com/Livox-SDK/Livox-SDK2>
...
### Full robot bringup (typical order)

```bash
# Static TF (IMU, LiDAR, wheels relative to base_link)
ros2 run myslam_car_driver robot_static_tf_pub_node

# Chassis driver — publishes /odom, /imu/data_raw, /imu/mag, TF odom→base_link
ros2 run myslam_car_driver driver_node

# RTK GPS driver — publishes gps/fix and gps/utmpos
ros2 launch um982_ros2_driver um982_ros2_driver.launch.py \
    port:=/dev/ttyUSB1 baud:=921600

# Livox LiDAR (MID360 example)
...
Edit `livox/src/livox_ros_driver2/config/MID360_config.json` to set the LiDAR and host IP addresses. Key launch parameters:
